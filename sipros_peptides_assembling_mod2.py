    ...
    if len(pro_greedy_list) > 0:
                ...
                for pep_list_one in pep_list:
                    ...
                    if pep_run_id in pep_data_dict:
                        ...
                        for pep_data_dict_one in pep_data_dict[pep_run_id]:
                            ...
                            each_balanced_spectrum_counts = divide(float(pep_spectral_count),
                                                                   float(each_balanced_pro_num))
        ...
        for pro_out_data_idx, pro_out_data_item in enumerate(pro_out_data):
            ...
            for pep_file, run_num in sorted(run_num_dict.items(), key=lambda x: x[1][-1]):
                ...
                nbsc_idx = first_nbsc_idx + inc_int * (
                        run_int - 1)  # calculate NormalizedBalancedSpectrumCounts column index
            ...
            if pro_ID.startswith('{'):
                ...
                for pep_file, run_num in sorted(run_num_dict.items(), key=lambda x: x[1][-1]):
                    ...
                    for psm_list_item in psm_list:
                        pro2psm_out_file.write('*\t' + '\t'.join(psm_list_item) + '\n')
    pro_out_file.close()
    pro2pep_out_file.close()
    pro2psm_out_file.close()
    return protein_fdr, target_proteins_after_filtering


## +------+
## | Main |
## +------+
def peptides_assembling(loc, argv=None):
    if argv is None:
        argv = sys.argv
    ...
    # Report output files
    protein_fdr, target_proteins_after_filtering = report_output(config_dict,
                                                                 run_num_dict,
                                                                 psm_run_num_dict,
                                                                 pep_data_dict,
                                                                 psm_data_dict,
...
def report_output_lite(config_dict,
                  pro_pep_dict,
                  pro_greedy_list):
    # save .pro.txt data
    pro_out_data = []
...
def peptides_assembling_lite(loc, argv=None):
    if argv is None:
        argv = sys.argv
    ...
    # pro2psm_out_file = open(base_out + ".pro2psm.txt", 'w')
    # Report output files
    protein_fdr, target_proteins_after_filtering = report_output_lite(config_dict, pro_pep_dict, pro_greedy_list)
    sys.stderr.write('Done!\n')
    print("protein_fdr:", protein_fdr)
    # Time record, calculate elapsed time, and display work end
    finish_time = datetime.now()
